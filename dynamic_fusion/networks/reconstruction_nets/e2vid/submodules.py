...
class RecurrentConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, recurrent_block_type="convlstm", activation="relu", norm=None):
    ...
    def forward(self, input_, prev_state=None):
        ...
        if prev_state is None:
            ...
            if state_size not in self.zero_tensors:
                ...
                self.zero_tensors[state_size] = (torch.zeros(state_size).to(input_.device), torch.zeros(state_size).to(input_.device))
